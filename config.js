...
// If Render/env URI has no database path we get "test" DB — force inventory_db
// Handles: ...mongodb.net?, ...mongodb.net, ...mongodb.net/?, ...mongodb.net/
const hasNoDb = MONGODB_URI.includes('.mongodb.net') && !MONGODB_URI.match(/\.mongodb\.net\/[a-zA-Z0-9_-]+/);
if (hasNoDb) {
  MONGODB_URI = MONGODB_URI.replace(/\.mongodb\.net(\/)?(\?|$)/, '.mongodb.net/inventory_db$2');
}
module.exports = {
  MONGODB_URI,
